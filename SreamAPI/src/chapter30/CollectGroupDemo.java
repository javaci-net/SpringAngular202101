...
public class CollectGroupDemo {
	public static void main(String[] args) {
		...
		Map<Integer, Long> map1 = Stream.of(names).map(e -> e.toUpperCase())
				.collect(Collectors.groupingBy(e -> e.length(), Collectors.counting()));
		System.out.println(map1);

		Map<Character, Long> map2 = Stream.of(names).map(e -> e.toUpperCase())
				.collect(Collectors.groupingBy(e -> e.charAt(0), TreeMap::new, Collectors.counting()));
		System.out.println(map2);

		map1.entrySet().stream().map(e -> e.getKey() + e.getValue());
		map1.forEach((k, v) -> {});
